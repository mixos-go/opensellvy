...
const dummy: PlatformPlugin = {
  ...
  auth: {
    ...
    exchangeCode: (_c, _code) => Promise.resolve({ accessToken: 't', refreshToken: 'r', expiresAt: Date.now() + 60_000 }),
    refreshToken: () => Promise.resolve({ accessToken: 't2', refreshToken: 'r2', expiresAt: Date.now() + 60_000 }),
  },
  gateway: {
    shop: { getProfile: () => Promise.resolve({ platformShopId: 'shop-1', shopName: 'Local Shop', marketplace: 'local' }), updateProfile: () => Promise.resolve() },
    order: { pull: () => Promise.resolve([]), get: () => Promise.reject(new Error('not implemented')), push: () => Promise.resolve(), update: () => Promise.resolve(), track: () => Promise.resolve([]) },
    product: { pull: () => Promise.resolve([]), push: () => Promise.resolve(), update: () => Promise.resolve(), listCategories: () => Promise.resolve([]) },
    inventory: { getStockLevels: () => Promise.resolve([]), sync: () => Promise.resolve(), adjust: () => Promise.resolve() },
    fulfillment: { ship: () => Promise.resolve(), updateStatus: () => Promise.resolve() },
    returns: { list: () => Promise.resolve([]), get: () => Promise.reject(new Error('not implemented')), act: () => Promise.resolve() },
    shipping: { getRates: () => Promise.resolve([]), listShipments: () => Promise.resolve([]), getShipment: () => Promise.reject(new Error('not implemented')) },
    payment: { list: () => Promise.resolve([]), get: () => Promise.reject(new Error('not implemented')), refund: () => Promise.resolve() },
    promotion: { list: () => Promise.resolve([]), get: () => Promise.reject(new Error('not implemented')), create: (c, p) => Promise.resolve(p), update: () => Promise.resolve(), setActive: () => Promise.resolve() },
    media: { upload: () => Promise.reject(new Error('not implemented')), list: () => Promise.resolve([]) },
    merchant: { getProfile: () => Promise.reject(new Error('not implemented')) },
  },
  webhook: {
